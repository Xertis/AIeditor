...
class DB_recent_files:
    ...
    def get(self, path: str, time_create: dt = None, time_edit: dt = None) -> recent_files:
        """ 
        Берёт файл из бд
        """
    ...
    def update_by_id(self, id: int, id_unsave_data: int=None, id_requests_history: int=None) -> bool:
        """
    ...
    def add(self, path: str, id_unsave_data: int=None, id_requests_history: int=None) -> recent_files:
        """
        Добавляет новый файл в дб
        """
        if not self.get_file_by_path(path=path):
            file = recent_files(path=path,                                    \
